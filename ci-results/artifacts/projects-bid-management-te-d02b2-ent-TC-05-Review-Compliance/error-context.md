...
Locator: getByRole('row').filter({ hasText: 'REF2026-0928' }).filter({ hasText: 'Verify Compliance' }).first()
Expected: visible
Timeout: 30000ms
...
Call log:
  ...
  - waiting for getByRole('row').filter({ hasText: 'REF2026-0928' }).filter({ hasText: 'Verify Compliance' }).first()
