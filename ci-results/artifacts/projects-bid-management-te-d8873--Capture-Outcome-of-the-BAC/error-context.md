...
Locator: getByRole('row').filter({ hasText: 'REF2026-0928' }).filter({ hasText: 'Capture outcome from the BAC' }).first()
Expected: visible
Timeout: 30000ms
...
Call log:
  ...
  - waiting for getByRole('row').filter({ hasText: 'REF2026-0928' }).filter({ hasText: 'Capture outcome from the BAC' }).first()
  ...
  - table:
    - row "Ref No Initiator Type Name Action Required Received Date Target Date Status Period In Possession":
      ...
      - columnheader "Period In Possession":
        ...
        - separator
    - rowgroup:
      - row "search REF2026-1097 Maand-awe Mamathuntsha Tender Process Tender REF2026-1097 - TC-01 Automated Draft Tender run-msimdegf - 90/10 Compulsory Hybrid Capture outcome from the BAC 07/08/2026 Adjudicate In Progress 28 day(s) ago":
        - cell "search":
          - link "search":
