# CLAUDE.md — Hybrid Markdown + Playwright Testing (Approvals project)

> **Multi-project hub.** This file describes the **Approvals** project. Shared Playwright + Allure infrastructure (`package.json`, `playwright.config.ts`, `node_modules/`, `scripts/run-plan.js`) lives at the hub root one level up. Tests run from the hub root: `node scripts/run-plan.js projects/Approvals/test-plans/<folder>/<plan>.md`.
...
2. `/RunTest` runs Playwright first: `node ../../scripts/run-plan.js projects/Approvals/test-plans/<folder>/<name>.md` (from hub root).
...
6. Regenerate the project dashboard (the hub's `scripts/build-project-dashboard.js --project=Approvals`) and the per-project Allure report.
...
1. Read this file (`projects/Approvals/CLAUDE.md`) completely
...
| Key | Value |
|-----|-------|
| App | Approvals Admin Portal |
| URL | https://pd-approvals-adminportal-qa.azurewebsites.net/ |
| Login URL | https://pd-approvals-adminportal-qa.azurewebsites.net/login |
| Environment | QA |

## Credentials
| Role | Username | Password |
|------|----------|----------|
| Standard user | Ian | 123qwe |

## Azure DevOps
| Key | Value |
|-----|-------|
| Organization | boxfusion |
| Project | pd-approvals |
| Test Plan ID | 100853 |
| Test Suite ID | 100854 |
| Suite URL | https://dev.azure.com/boxfusion/pd-approvals/_testPlans/define?planId=100853&suiteId=100854 |
