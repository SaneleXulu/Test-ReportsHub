// AUTO-RECORDED from test-plans/Login/invalid-login.md
// Source: Azure DevOps test plan #100853, suite #100854, test case #104705
...
const INVALID_USER = { username: 'invaliduser', password: 'wrongpass123' };

test('TC-01 — Invalid (Username and Password)', async ({ page }) => {
  ...
  await page.getByPlaceholder(/username/i).fill(INVALID_USER.username);

  // STEP 4: TYPE Password field with `wrongpass123`
  await page.getByPlaceholder(/password/i).fill(INVALID_USER.password);

  // ASSERT Username/Password fields accept the entered invalid values
  await expect(page.getByPlaceholder(/username/i)).toHaveValue(INVALID_USER.username);
  await expect(page.getByPlaceholder(/password/i)).toHaveValue(INVALID_USER.password);
