// AUTO-RECORDED from test-plans/Memo/verify-cc-user-in-routing-list.md
// Source: Azure DevOps test plan #100853, suite #100854, test case #102664
...
// Types a search filter into an open Ant Design Select dropdown and picks the first matching option
// via keyboard, avoiding the virtual-list visibility flakiness of clicking a specific option element.
async function selectDropdownOptionByText(page: Page, text: string) {
  await page.keyboard.type(text);
  await expect(page.getByRole('option').first()).toHaveCount(1, { timeout: 10_000 });
  await page.keyboard.press('ArrowDown');
  await page.keyboard.press('Enter');
}

test('TC-01 — Validate system can submit when CC\'d user is also a routing signatory', async ({ page }) => {
  ...
  // STEP 8: POPULATE all mandatory fields, adding "Admire" under the CC field
  ...
  await selectDropdownOptionByText(page, 'Admire');

  // ASSERT (BLOCKING) Admire is displayed as the CC signatory
  const ccContainer = ccField.locator('xpath=../..');
  await expect(ccContainer).toContainText(/admire/i, { timeout: 10_000 });
  ...
  // STEP 9: ACTION the memo through Next (Compose -> Attachments) and Next (Attachments -> Routing)
  ...
  // ASSERT (BLOCKING) The Routing step is reached
  ...
  // STEP 10: CLICK the Select Signatory dropdown.
  // Clicking the placeholder <span> directly is intercepted by its own sibling search <input> (same
  // Ant Design Select quirk as the CC field) — target the combobox role instead, same fix as CC.
  const approverField = page.getByRole('combobox').first();
  await approverField.click();

  // STEP 11: SELECT "Admire" as a signatory
  // ASSERT (BLOCKING) The dropdown lists Admire and selecting it displays Admire
  await selectDropdownOptionByText(page, 'Admire');
  ...
  // ASSERT (BLOCKING) Admire is added to the routing table
  ...
  // ASSERT (BLOCKING) The memo routes to the Confirmation step
  await expect(page.getByRole('button', { name: /submit/i })).toHaveCount(0, { timeout: 20_000 });
});
