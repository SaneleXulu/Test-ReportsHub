// AUTO-RECORDED from test-plans/Memo/verify-close-memo.md
// Source: Azure DevOps test plan #100853, suite #100854, test case #105864
...
test('TC-01 — Verify Close Memo', async ({ page }) => {
  ...
  // AI-repair (confirmed live): the "Close" control on this screen is a link (icon + text "Close"),
  // not a button, with href /dynamic/Shesha.Workflow/workflows-my-items. It's the only role=link whose
  // accessible name contains "close" (other "close" hits on this page are icons/buttons: the header's
  // draft-close glyph and the dismissable Hint alert's close button).
  const closeButton = page.getByRole('link', { name: /close/i });
  await expect(closeButton).toBeVisible({ timeout: 15_000 });

  // STEP 20: CLICK the "Close" button
  await closeButton.click();

  // ASSERT (BLOCKING) The memo page closes and the app navigates back to the My Items index table
  // (workflow-action screen is gone; the My Items list — Create New button + index grid — is visible).
  await page.waitForURL(url => !/workflow-action/.test(url.toString()), { timeout: 20_000 });
  await expect(page.getByRole('button', { name: /create new/i })).toBeVisible({ timeout: 15_000 });
});
