// AUTO-RECORDED from test-plans/Memo/verify-comments-mandatory-for-negative-actions-craig.md
// Source: Azure DevOps test plan #100853, suite #100854, test case #105910
...
test('TC-01 — Verify Comments Are Mandatory For Negative Actions', async ({ page }) => {
  ...
  // ASSERT (BLOCKING) Opening the item shows a "Do not Recommend" radio option
  ...
  // STEP 20: SELECT the "Do not Recommend" radio button
  await doNotRecommendRadio.check();
  await expect(doNotRecommendRadio).toBeChecked();

  // AI-repair: dropped a brittle intermediate check on the "Supporting Comments" label's own visibility
  // state (it resolved to a real DOM element flagged "hidden" by Playwright, likely due to how Ant
  // Design's required-mark CSS is applied here) — not essential to what this test actually validates.
  // The real pass/fail criterion is the mandatory-comments error appearing after Submit, checked below.

  // STEP 21: CLICK the Submit button WITHOUT populating Comments.
