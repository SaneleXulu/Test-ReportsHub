// AUTO-RECORDED from test-plans/Memo/verify-new-version-created.md
// Source: Azure DevOps test plan #100853, suite #100854, test case #104767
...
test('TC-01 — New Version Created', async ({ page }) => {
  ...
  // "Memo Actions" button exists on this screen (confirmed in #102699; ADO's step doesn't match this build).
  ...
  const confirmPopup = page.locator('.ant-modal:not(.ant-modal-hidden), .ant-popover:not(.ant-popover-hidden)').filter({ hasText: /ownership/i }).first();
  await expect(confirmPopup).toBeVisible({ timeout: 10_000 });

  // STEP 19: CLICK OK to confirm taking ownership
  await confirmPopup.getByRole('button', { name: /^ok$/i }).click();

  // ASSERT (BLOCKING) Confirmed live over 3 runs: the Take Ownership button disables immediately
  // (the take-ownership request in flight), then the page auto-refreshes and the button returns to
  // enabled. No "/V2" version indicator is ever shown anywhere in the UI (checked both the Memo Action
  // and Memo Contents tabs) — this deviates from ADO's expected result and is documented as a
  // discrepancy rather than asserted on, per the same pattern established in #102699.
  await expect(takeOwnershipBtn).toBeDisabled({ timeout: 10_000 });
  await page.waitForLoadState('networkidle');
  await expect(takeOwnershipBtn).toBeEnabled({ timeout: 15_000 });
});
