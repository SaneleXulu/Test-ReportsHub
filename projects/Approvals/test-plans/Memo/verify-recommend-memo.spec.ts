// AUTO-RECORDED from test-plans/Memo/verify-recommend-memo.md
// Source: Azure DevOps test plan #100853, suite #100854, test case #104791
...
test('TC-01 — Verify Recommend Memo', async ({ page }) => {
  ...
  // A fresh login resets the view-mode control back to "Live" — repeat the Live -> Latest switch for
  // Craig's session, same retry-for-popover-race pattern as Ian's switch in STEP 2.
  for (let attempt = 0; attempt < 3; attempt++) {
    await viewModeControl.click();
    await page.waitForTimeout(300);
    await page.getByText('Latest', { exact: true }).click();
  ...
  // ASSERT (BLOCKING) Opening the item shows a "Recommend" radio option
  ...
  // The screen offers both "Recommend" and "Do not Recommend" radios — match exactly to avoid ambiguity.
  ...
  // STEP 20: CLICK the "Recommend" radio button
  await recommendRadio.check();
  await expect(recommendRadio).toBeChecked();

  // This screen has a mandatory "Supporting Comments" field (not present on the plain Approve/Decline
  // screen) — populate it before submitting, otherwise the Submit click/validation hangs.
  const supportingComments = page.getByPlaceholder(/start typing/i).or(page.getByLabel(/supporting comments/i));
  await supportingComments.first().fill('Test recommendation comment');

  // STEP 21: CLICK the Submit button. Confirmed live: there is no toast/dialog "success" message — the
  // page navigates directly to the completed item view.
  const submitBtn = page.getByRole('button', { name: /submit/i }).last();
  await submitBtn.scrollIntoViewIfNeeded();
  await submitBtn.click();

  // ASSERT (BLOCKING) The item moves to the next step: the header badge changes to "COMPLETED" and the
  // Memo Actions timeline shows "Craig M: Sales Director: Recommend" with the submitted comment.
  await expect(page.getByText(/^completed$/i).first()).toBeVisible({ timeout: 20_000 });
  await expect(page.getByText(/craig m.*sales director/i).first()).toBeVisible();
  await expect(page.getByText(/recommend/i).first()).toBeVisible();
  await expect(page.getByText('Test recommendation comment', { exact: true })).toBeVisible();
});
