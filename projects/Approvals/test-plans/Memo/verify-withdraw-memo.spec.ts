// AUTO-RECORDED from test-plans/Memo/verify-withdraw-memo.md
// Source: Azure DevOps test plan #100853, suite #100854, test case #105855
...
test('TC-01 — Verify Withdraw Memo', async ({ page }) => {
  ...
  // STEP 20: CLICK the "Withdraw" button. Confirmed live in #105864: this screen has a plain
  // role=button "Withdraw" alongside Recommend/Do not Recommend, Submit, Take Ownership, Send To
  // Approver First, Send Back and the Close link.
  ...
  // ASSERT (BLOCKING) Withdraw Memo dialog appears with Cancel and OK buttons.
  // TODO[selector]: dialog title/Comments field not yet confirmed live — using a role=dialog scope with
  // a generic Comments textbox/textarea lookup and OK/Cancel buttons scoped to the dialog.
  const withdrawDialog = page.getByRole('dialog').filter({ hasText: /withdraw/i });
  await expect(withdrawDialog).toBeVisible({ timeout: 10_000 });
  const okButton = withdrawDialog.getByRole('button', { name: /^ok$/i });
  const cancelButton = withdrawDialog.getByRole('button', { name: /cancel/i });
  await expect(cancelButton).toBeVisible();
  await expect(okButton).toBeVisible();

  // STEP 21: POPULATE the Comments field on the Withdraw Memo dialog
  const commentsField = withdrawDialog.getByRole('textbox').last();
  await commentsField.fill('Test withdraw comment');

  // ASSERT (BLOCKING) OK is enabled once Comments is populated
  await expect(okButton).toBeEnabled({ timeout: 10_000 });

  // STEP 22: CLICK the OK button
  await okButton.click();

  // ASSERT (BLOCKING) The item auto-refreshes and its status changes to "Withdrawn"
