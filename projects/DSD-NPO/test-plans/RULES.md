...
> ```
> node scripts/run-plan.js projects/DSD-NPO/test-plans/<folder>/<plan>.md
> node scripts/build-project-data.js --project=DSD-NPO
> ```
>
...
## 4. Report Format
Reports saved to `test-reports/YYYY-MM-DD/<plan-name>.md` (relative to the project root, i.e. `projects/DSD-NPO/test-reports/...`):
```
# Report: <Plan Title>
...
### 5.1 Report front-matter is machine-read — three rules
The dashboard parses each report's front-matter. Break any of these and the run silently vanishes from the
dashboard instead of erroring.

0. **The line must NOT be bulleted.** The parser regex `/^\*\*([A-Za-z ]+):\*\*\s*(.+?)\s*$/` is anchored at line
   start, so a front matter written as a markdown list — `- **Result:** PASSED` — matches **nothing** and the
   report parses with no Result at all. On PD-PMDS this silently mis-classified 10 runs as `UNKNOWN`. Write
   `**Result:** …`, never `- **Result:** …`.
   ⚠️ And **`PASS` is not `PASSED`** — the regex needs the whole token. `✅ PASS` cost another 34 runs there.
...
2. **`**Plan:**` MUST be the bare plan path, nothing else.** Runs are indexed by that exact string, so
   `…/register-new-npo.md (TC-20)` creates a *phantom flow* — the run detaches from its real plan and
...
node scripts/build-project-data.js --project=DSD-NPO
```
The dashboard reads `projects/DSD-NPO/data.js`; `index.html` is only a thin shell that renders it.
...
```bash
rm -rf projects/DSD-NPO/allure-results
node scripts/generate-allure-results.js --project=DSD-NPO
npx allure generate projects/DSD-NPO/allure-results --clean -o projects/DSD-NPO/allure-report
```
...
### Execution flow
1. `node scripts/run-plan.js projects/DSD-NPO/test-plans/<folder>/<plan>.md` — runs Playwright, emits a JSON summary.
