# CLAUDE.md — Hybrid Markdown + Playwright Testing (ITS project)

> **Multi-project hub.** This file describes the **ITS** project. Shared Playwright + Allure infrastructure (`package.json`, `playwright.config.ts`, `node_modules/`, `scripts/run-plan.js`) lives at the hub root one level up. Tests run from the hub root: `node scripts/run-plan.js projects/ITS/test-plans/<folder>/<plan>.md`.
...
2. `/RunTest` runs Playwright first: `node ../../scripts/run-plan.js projects/ITS/test-plans/<folder>/<name>.md` (from hub root).
...
6. Regenerate the project dashboard (the hub's `scripts/build-project-dashboard.js --project=ITS`) and the per-project Allure report.
...
1. Read this file (`projects/ITS/CLAUDE.md`) completely
...
| Key | Value |
|-----|-------|
| App | ITS Admin Portal |
| URL | https://pd-invtracking-adminportal-qa.azurewebsites.net/ |
| Login URL | https://pd-invtracking-adminportal-qa.azurewebsites.net/login |
| Environment | QA |
...
| Test Plan ID | 102133 |
| Test Suite ID | 102355 |
| Suite URL | https://dev.azure.com/boxfusion/PD-Shesha%203%20Migration/_testPlans/define?planId=102133&suiteId=102355 |
