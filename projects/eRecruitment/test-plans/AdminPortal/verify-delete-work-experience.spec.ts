// AUTO-RECORDED from test-plans/AdminPortal/verify-delete-work-experience.md
// Source: Azure DevOps project pd-recruitment, test case #106551 "Verify
// Delete Work Experience" (work item rev 3). The .md plan is canonical.
...
// (currently listed as "Edit Last Name F"), Work Experience panel.
//
// BUG under test: confirmed live 2026-08-05 that no Work Experience row has
// a Delete (trash) icon, in view mode or inline-edit mode — only Edit, and
// once in edit mode: Save/Cancel/date-clear icons. TC-07 asserts the
// presence of a .anticon-delete icon and is expected to fail, proving the
// defect (same class as ADMINPORTAL-106543/106544's missing Qualification
// Status field). TC-08-10 are skipped as blocked. No data is modified by
// this spec.
...
test.describe('ADMINPORTAL-106551 — Verify Delete Work Experience', () => {
  ...
  test('TC-06: Open the target application', async ({ page }) => {
    ...
    // ASSERT (BLOCKING) status badge shows "PRE-SCREENED" — confirmed live
    // 2026-08-05 the application has since progressed past the
    // "AWAITING PRE-SCREENING" state documented in ADMINPORTAL-106529/106538
    // (application state moves forward over time; that note is now stale).
    // This matches ADO's expected wording.
  ...
  test('TC-07: Navigate to Work Experience panel and attempt to click the Delete icon', async ({ page }) => {
    ...
    // giving the defect a genuine automated signal instead of a manual note.
