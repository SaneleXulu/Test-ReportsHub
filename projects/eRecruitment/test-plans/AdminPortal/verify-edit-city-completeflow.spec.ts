// AUTO-RECORDED from test-plans/AdminPortal/verify-edit-city-completeflow.md
// Source: Azure DevOps project pd-recruitment, test case #106270 "Verify
// Edit City" (work item rev 3). The .md plan is canonical. AI-repair
...
// Current City value is "Pretoria" (set during the #106172 wizard). ADO
// step 10's wording is garbled ("natire of disability text area") but the
// field and test title make the intent unambiguous (City) — same class of
// copy/paste error seen across this batch (e.g. ADMINPORTAL-106269).
//
...
// ADMINPORTAL-106240/106246/106247/106251/106255/106256/106257/106262/106269).
...
//
// TC-10 clicks Save for real, changing City from "Pretoria" to "Edited
// City" — confirmed with the requester before running. All earlier TCs
// never click Save.
...
const CURRENT_CITY = 'Pretoria';
const NEW_CITY = 'Edited City';
...
function cityInput(page: Page) {
  return page.locator('label', { hasText: 'City' }).first().locator('xpath=../..').locator('input').first();
}
...
test.describe('ADMINPORTAL-106270 — Verify Edit City', () => {
  ...
  test('TC-06: Open the application created on Test Case 106172', async ({ page }) => {
    ...
    // ASSERT (BLOCKING) Personal Details panel visible with the current City
    await expect(page.getByText('Personal Details', { exact: true }).first()).toBeVisible();
    await expect(page.getByText(CURRENT_CITY, { exact: true }).first()).toBeVisible();
  ...
  test('TC-07: Click Edit Personal Details', async ({ page }) => {
    ...
    // ASSERT (BLOCKING) City field is now an editable input, Save button visible
    await expect(cityInput(page)).toBeVisible();
    await expect(saveButton(page)).toBeVisible();
  });

  test('TC-08: Clear the City field', async ({ page }) => {
    test.setTimeout(180_000);
    await loginAsKwena(page);
    await openTargetApplication(page);
    await clickEditPersonalDetails(page);
    ...
    await input.click();
    await input.fill('');
    // ASSERT (BLOCKING) City field is empty
    await expect(input).toHaveValue('');
  });

  test('TC-09: Enter "Edited City"', async ({ page }) => {
    test.setTimeout(180_000);
    await loginAsKwena(page);
    await openTargetApplication(page);
    await clickEditPersonalDetails(page);
    ...
    await input.click();
    await input.fill('');
    await input.fill(NEW_CITY);
    // ASSERT (BLOCKING) City field contains the typed value
    await expect(input).toHaveValue(NEW_CITY);
  });
  ...
  test('TC-10: Click on Save', async ({ page }) => {
    ...
    // ASSERT (BLOCKING) after Save + reload, City shows the updated value
    await page.reload({ waitUntil: 'domcontentloaded' });
    await page.waitForLoadState('networkidle').catch(() => {});
    await page.waitForTimeout(2000);
    await expect(page.getByText(NEW_CITY, { exact: true }).first()).toBeVisible({ timeout: 15_000 });

    // ASSERT (BLOCKING) the new City also appears in the Applications index table
