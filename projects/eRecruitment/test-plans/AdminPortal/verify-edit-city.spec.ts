// AUTO-RECORDED from test-plans/AdminPortal/verify-edit-city.md
// Source: Azure DevOps project pd-recruitment, test case #106538 "Verify
// Edit City" (work item rev 2). The .md plan is canonical. AI-repair
...
// ADO. ADO steps 10-11 have copy/paste leftovers from earlier test cases
// in this family ("...natire of disability text area", "Updated Province
// is displayed") — read as "City". TC-10 clicks Save for real,
// permanently changing the candidate's City (currently blank) —
...
function cityInput(page: Page) {
  return page.locator('label', { hasText: 'City' }).locator('xpath=../..').locator('input, textarea').first();
}
...
test.describe('ADMINPORTAL-106538 — Verify Edit City', () => {
  ...
  test('TC-10: Click on Save', async ({ page }) => {
    ...
    await page.waitForLoadState('networkidle').catch(() => {});
    await page.waitForTimeout(2000);
    await expect(page.getByText(NEW_CITY, { exact: false }).first()).toBeVisible({ timeout: 15_000 });
