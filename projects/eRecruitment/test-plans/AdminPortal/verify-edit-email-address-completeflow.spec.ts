// AUTO-RECORDED from test-plans/AdminPortal/verify-edit-email-address-completeflow.md
// Source: Azure DevOps project pd-recruitment, test case #106251 "Verify
// Edit Email Address" (work item rev 2). The .md plan is canonical.
...
// ADMINPORTAL-106240/106246/106247). This spec waits 15s after clicking
// Save and retries once if still in edit mode. See
// test-reports/bugs/2026-08-05-edit-personal-details-save-silently-fails.md.
//
// TC-11 clicks Save for real, changing Email Address from
// "Reuben.mashifane@boxfusion.io" to "Edit.Email@test.com" — confirmed
// with the requester before running. All earlier TCs never click Save.
...
const CURRENT_EMAIL = 'Reuben.mashifane@boxfusion.io';
const NEW_EMAIL = 'Edit.Email@test.com';
...
function emailInput(page: Page) {
  return page.locator('label', { hasText: 'Email Address' }).first().locator('xpath=../..').locator('input').first();
}
...
test.describe('ADMINPORTAL-106251 — Verify Edit Email Address', () => {
  ...
  test('TC-06: Open the application created on Test Case 106172', async ({ page }) => {
    ...
    // ASSERT (BLOCKING) Personal Details panel visible with the current Email Address
    await expect(page.getByText('Personal Details', { exact: true }).first()).toBeVisible();
    await expect(page.getByText(CURRENT_EMAIL, { exact: false }).first()).toBeVisible();
  ...
  test('TC-07: Click Edit Personal Details', async ({ page }) => {
    ...
    // ASSERT (BLOCKING) Email Address field is now an editable input, Save button visible
    await expect(emailInput(page)).toBeVisible();
    await expect(saveButton(page)).toBeVisible();
  });

  test('TC-08: Click inside the Email Address field', async ({ page }) => {
    test.setTimeout(180_000);
    await loginAsKwena(page);
    await openTargetApplication(page);
    await clickEditPersonalDetails(page);
    ...
    await input.click();
    // ASSERT (BLOCKING) Email Address field is focused
    await expect(input).toBeFocused();
  });

  test('TC-09: Clear the Email Address field', async ({ page }) => {
    test.setTimeout(180_000);
    await loginAsKwena(page);
    await openTargetApplication(page);
    await clickEditPersonalDetails(page);
    ...
    await input.click();
    await input.fill('');
    // ASSERT (BLOCKING) Email Address field is empty
    await expect(input).toHaveValue('');
  });

  test('TC-10: Enter "Edit.Email@test.com"', async ({ page }) => {
    test.setTimeout(180_000);
    await loginAsKwena(page);
    await openTargetApplication(page);
    await clickEditPersonalDetails(page);
    ...
    await expect(input).toHaveValue(NEW_EMAIL);
  });
  ...
  test('TC-11: Click on Save', async ({ page }) => {
    ...
    // ASSERT (BLOCKING) after Save + reload, Email Address shows the updated value
    await page.reload({ waitUntil: 'domcontentloaded' });
    await page.waitForLoadState('networkidle').catch(() => {});
    await page.waitForTimeout(2000);
    await expect(page.getByText(NEW_EMAIL, { exact: true }).first()).toBeVisible({ timeout: 15_000 });
  });
});
