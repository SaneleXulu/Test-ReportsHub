// AUTO-RECORDED from test-plans/AdminPortal/verify-edit-email-address.md
// Source: Azure DevOps project pd-recruitment, test case #106531 "Verify
...
// Email Address — confirmed with the requester before running. All
...
function emailAddressInput(page: Page) {
  return page.locator('label', { hasText: 'Email Address' }).locator('xpath=../..').locator('input').first();
}
...
test.describe('ADMINPORTAL-106531 — Verify Edit Email Address', () => {
  ...
  test('TC-08: Click inside the Email Address field', async ({ page }) => {
    ...
    await openTargetApplication(page);
    await clickEditPersonalDetails(page);
    const input = emailAddressInput(page);
    await input.click();
    // ASSERT (BLOCKING) Email Address field is focused and editable
  ...
  test('TC-11: Click on Save', async ({ page }) => {
    ...
    await page.waitForLoadState('networkidle').catch(() => {});
    await page.waitForTimeout(2000);
    await expect(page.getByText(NEW_EMAIL, { exact: false }).first()).toBeVisible({ timeout: 15_000 });
  });
});
