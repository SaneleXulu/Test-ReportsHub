// AUTO-RECORDED from test-plans/AdminPortal/verify-edit-employer-completeflow.md
// Source: Azure DevOps project pd-recruitment, test case #106295 "Verify
// Edit Employer" (work item rev 2). The .md plan is canonical. AI-repair
...
// ADMINPORTAL-106285) / "Test Employer Pty Ltd".
//
...
// (same fix as ADMINPORTAL-106540/106541/106542/106547/106285).
...
test.describe('ADMINPORTAL-106295 — Verify Edit Employer', () => {
  ...
  test('TC-09: Clear the Employer text area', async ({ page }) => {
    test.setTimeout(180_000);
    await loginAsKwena(page);
    await openTargetApplication(page);
    const index = await enterWorkExperienceEditMode(page);
    ...
    await input.click();
    await input.fill('');
    // ASSERT (BLOCKING) Employer field is empty
    await expect(input).toHaveValue('');
  });

  test('TC-10: Populate the field with "Edited Employer"', async ({ page }) => {
    test.setTimeout(180_000);
    await loginAsKwena(page);
    await openTargetApplication(page);
    const index = await enterWorkExperienceEditMode(page);
  ...
  test('TC-11: Click on Save', async ({ page }) => {
    ...
    await page.waitForTimeout(2000);
    // ASSERT (BLOCKING) after Save + reload, the row shows the updated Employer
