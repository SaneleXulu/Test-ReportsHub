// AUTO-RECORDED from test-plans/AdminPortal/verify-edit-gender-completeflow.md
// Source: Azure DevOps project pd-recruitment, test case #106256 "Verify
// Edit Gender" (work item rev 3). The .md plan is canonical. AI-repair
...
// Gender dropdown options confirmed via ADMINPORTAL-106533 (the other
// candidate): "Female" and "Not Disclosed" when "Male" is selected.
//
...
// ADMINPORTAL-106240/106246/106247/106251/106255). This spec waits 15s
// after clicking Save and retries once if still in edit mode. See
// test-reports/bugs/2026-08-05-edit-personal-details-save-silently-fails.md.
//
// TC-10 clicks Save for real, changing Gender from "Male" to "Not
// Disclosed" — confirmed with the requester before running. All earlier
...
function genderSelect(page: Page) {
  return page.locator('label', { hasText: 'Gender' }).first().locator('xpath=../..').locator('.ant-select').first();
}
...
test.describe('ADMINPORTAL-106256 — Verify Edit Gender', () => {
  ...
  test('TC-10: Click on Save', async ({ page }) => {
    ...
    if (await genderSelect(page).isVisible().catch(() => false)) {
      await saveBtn.click();
      await page.waitForTimeout(15_000);
    }
