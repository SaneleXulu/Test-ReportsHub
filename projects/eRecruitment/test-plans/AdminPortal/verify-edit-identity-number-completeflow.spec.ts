// AUTO-RECORDED from test-plans/AdminPortal/verify-edit-identity-number-completeflow.md
// Source: Azure DevOps project pd-recruitment, test case #106247 "Verify
// Edit Identity Number" (work item rev 4). The .md plan is canonical.
...
// in edit mode; one attempt needed a ~15s wait before the save actually
// went through (confirmed in ADMINPORTAL-106240/106246). This spec waits
...
//
// TC-11 clicks Save for real, changing Identity Number from
// "9401155123095" to "8803055678089" (a freshly computed, checksum-valid
// SA ID number) — confirmed with the requester before running. All
...
const CURRENT_ID_NUMBER = '9401155123095';
const NEW_ID_NUMBER = '8803055678089';
...
function idNumberInput(page: Page) {
  return page.locator('label', { hasText: 'Identity Number' }).first().locator('xpath=../..').locator('input').first();
}
...
test.describe('ADMINPORTAL-106247 — Verify Edit Identity Number', () => {
  ...
  test('TC-06: Open the application created on Test Case 106172', async ({ page }) => {
    ...
    // ASSERT (BLOCKING) Personal Details panel visible with the current Identity Number
    await expect(page.getByText('Personal Details', { exact: true }).first()).toBeVisible();
    await expect(page.getByText(CURRENT_ID_NUMBER, { exact: true }).first()).toBeVisible();
  ...
  test('TC-07: Click Edit Personal Details', async ({ page }) => {
    ...
    // ASSERT (BLOCKING) Identity Number field is now an editable input, Save button visible
    await expect(idNumberInput(page)).toBeVisible();
    await expect(saveButton(page)).toBeVisible();
  });

  test('TC-08: Click inside the Identity Number field', async ({ page }) => {
    test.setTimeout(180_000);
    await loginAsKwena(page);
    await openTargetApplication(page);
    await clickEditPersonalDetails(page);
    const input = idNumberInput(page);
    await input.click();
    // ASSERT (BLOCKING) Identity Number field is focused
    await expect(input).toBeFocused();
  });

  test('TC-09: Clear the Identity Number field', async ({ page }) => {
    test.setTimeout(180_000);
    await loginAsKwena(page);
    await openTargetApplication(page);
    await clickEditPersonalDetails(page);
    ...
    await input.click();
    await input.fill('');
    // ASSERT (BLOCKING) Identity Number field is empty
    await expect(input).toHaveValue('');
  });

  test('TC-10: Enter a different valid 13-digit ID Number', async ({ page }) => {
    test.setTimeout(180_000);
    await loginAsKwena(page);
    await openTargetApplication(page);
    await clickEditPersonalDetails(page);
    ...
    await input.click();
    await input.fill('');
    await input.fill(NEW_ID_NUMBER);
    // ASSERT (BLOCKING) Identity Number field contains the typed value, no validation error
    await expect(input).toHaveValue(NEW_ID_NUMBER);
    await expect(page.getByText('Invalid ID Number', { exact: false })).toHaveCount(0);
  });
  ...
  test('TC-11: Click on Save', async ({ page }) => {
    ...
    // ASSERT (BLOCKING) after Save + reload, Identity Number shows the updated value
    await page.reload({ waitUntil: 'domcontentloaded' });
    await page.waitForLoadState('networkidle').catch(() => {});
    await page.waitForTimeout(2000);
    await expect(page.getByText(NEW_ID_NUMBER, { exact: true }).first()).toBeVisible({ timeout: 15_000 });

    // ASSERT (BLOCKING) the Applications index table also reflects the new ID
