// AUTO-RECORDED from test-plans/AdminPortal/verify-edit-identity-number.md
// Source: Azure DevOps project pd-recruitment, test case #106530 "Verify
// Edit Identity Number" (work item rev 2). The .md plan is canonical.
...
// Surname/Initials value actually is, same pattern as #106529's spec).
// Confirmed live 2026-08-05: application opens as "AWAITING
// PRE-SCREENING", not "pre-screened" per ADO. ADO calls the field
// "ID Number" but the actual on-screen label is "Identity Number".
// TC-11 clicks Save for real, permanently changing the candidate's
// Identity Number — confirmed with the requester before running. All
...
const NEW_IDENTITY_NUMBER = '9203015800086';
...
function identityNumberInput(page: Page) {
  return page.locator('label', { hasText: 'Identity Number' }).locator('xpath=../..').locator('input').first();
}
...
test.describe('ADMINPORTAL-106530 — Verify Edit Identity Number', () => {
  ...
  test('TC-07: Click Edit Personal Details', async ({ page }) => {
    ...
    await expect(identityNumberInput(page)).toBeVisible();
  ...
  test('TC-08: Click inside the Identity Number field', async ({ page }) => {
    ...
    await openTargetApplication(page);
    await clickEditPersonalDetails(page);
    const input = identityNumberInput(page);
    await input.click();
    // ASSERT (BLOCKING) Identity Number field is focused and editable
  ...
  test('TC-10: Enter a different valid 13-digit Identity Number', async ({ page }) => {
    await loginAsKwena(page);
    await openTargetApplication(page);
    await clickEditPersonalDetails(page);
    const input = identityNumberInput(page);
    await input.click();
    await input.fill('');
    await input.fill(NEW_IDENTITY_NUMBER);
    // ASSERT (BLOCKING) Identity Number field contains the typed value
    await expect(input).toHaveValue(NEW_IDENTITY_NUMBER);
  });
  ...
  test('TC-11: Click on Save', async ({ page }) => {
    ...
    await page.waitForLoadState('networkidle').catch(() => {});
    await page.waitForTimeout(2000);
    await expect(page.getByText(NEW_IDENTITY_NUMBER, { exact: false }).first()).toBeVisible({ timeout: 15_000 });

    // ASSERT (BLOCKING) the new Identity Number also appears in the Applications index table
    await openTargetJobPosting(page);
    await page.mouse.wheel(0, 1500);
