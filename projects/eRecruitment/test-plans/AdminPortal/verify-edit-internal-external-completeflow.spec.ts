// AUTO-RECORDED from test-plans/AdminPortal/verify-edit-internal-external-completeflow.md
// Source: Azure DevOps project pd-recruitment, test case #106257 "Verify
// Edit internal/External Applicant" (work item rev 3). The .md plan is
...
// Internal/External dropdown options confirmed via ADMINPORTAL-106534 (the
// other candidate): "External Applicant", "Past Employee", "Recruitment
// Agency" (and "Current Employee"). Our current value is "External
// Applicant".
//
...
// ADMINPORTAL-106240/106246/106247/106251/106255/106256). This spec waits
// 15s after clicking Save and retries once if still in edit mode. See
// test-reports/bugs/2026-08-05-edit-personal-details-save-silently-fails.md.
//
// TC-10 clicks Save for real, changing Internal/External from "External
// Applicant" to "Past Employee" — confirmed with the requester before
...
const NEW_APPLICANT_TYPE = 'Past Employee';
...
function internalExternalSelect(page: Page) {
  return page.locator('label', { hasText: 'Internal/External' }).first().locator('xpath=../..').locator('.ant-select').first();
}
...
async function selectPastEmployee(page: Page) {
  await internalExternalSelect(page).click();
  await page.waitForTimeout(500);
  await page.getByText(NEW_APPLICANT_TYPE, { exact: true }).first().click();
  await page.waitForTimeout(500);
}

test.describe('ADMINPORTAL-106257 — Verify Edit internal/External Applicant', () => {
  ...
  test('TC-07: Click Edit Personal Details', async ({ page }) => {
    ...
    // ASSERT (BLOCKING) Internal/External dropdown is now editable, Save button visible
    await expect(internalExternalSelect(page)).toBeVisible();
    await expect(saveButton(page)).toBeVisible();
  });

  test('TC-08: Click on the Internal/External dropdown', async ({ page }) => {
    test.setTimeout(180_000);
    await loginAsKwena(page);
    await openTargetApplication(page);
    await clickEditPersonalDetails(page);
    await internalExternalSelect(page).click();
    await page.waitForTimeout(500);
    // ASSERT (BLOCKING) application type options are displayed
    await expect(page.getByText(NEW_APPLICANT_TYPE, { exact: true }).first()).toBeVisible();
    await expect(page.getByText('Recruitment Agency', { exact: true }).first()).toBeVisible();
  });

  test('TC-09: Select "Past Employee"', async ({ page }) => {
    test.setTimeout(180_000);
    await loginAsKwena(page);
    await openTargetApplication(page);
    await clickEditPersonalDetails(page);
    await selectPastEmployee(page);
    // ASSERT (BLOCKING) Internal/External dropdown now shows "Past Employee"
    await expect(internalExternalSelect(page)).toContainText(NEW_APPLICANT_TYPE);
  });
  ...
  test('TC-10: Click on Save', async ({ page }) => {
    ...
    if (await internalExternalSelect(page).isVisible().catch(() => false)) {
      await saveBtn.click();
      await page.waitForTimeout(15_000);
    }
    // ASSERT (BLOCKING) after Save + reload, Internal/External shows the updated value
    await page.reload({ waitUntil: 'domcontentloaded' });
    await page.waitForLoadState('networkidle').catch(() => {});
    await page.waitForTimeout(2000);
    await expect(page.getByText(NEW_APPLICANT_TYPE, { exact: false }).first()).toBeVisible({ timeout: 15_000 });

    // ASSERT (BLOCKING) the new Internal/External status also appears in the Applications index table
