// AUTO-RECORDED from test-plans/AdminPortal/verify-edit-mobile-number-completeflow.md
// Source: Azure DevOps project pd-recruitment, test case #106255 "Verify
// Edit mobile number" (work item rev 3). The .md plan is canonical.
...
// ADMINPORTAL-106240/106246/106247/106251). This spec waits 15s after
// clicking Save and retries once if still in edit mode. See
// test-reports/bugs/2026-08-05-edit-personal-details-save-silently-fails.md.
//
// TC-11 clicks Save for real, changing Mobile Number from "0876543245" to
// "0821234567" — confirmed with the requester before running. All earlier
...
const CURRENT_MOBILE = '0876543245';
const NEW_MOBILE = '0821234567';
...
function mobileInput(page: Page) {
  return page.locator('label', { hasText: 'Mobile Number' }).first().locator('xpath=../..').locator('input').first();
}
...
test.describe('ADMINPORTAL-106255 — Verify Edit mobile number', () => {
  ...
  test('TC-06: Open the application created on Test Case 106172', async ({ page }) => {
    ...
    // ASSERT (BLOCKING) Personal Details panel visible with the current Mobile Number
    await expect(page.getByText('Personal Details', { exact: true }).first()).toBeVisible();
    await expect(page.getByText(CURRENT_MOBILE, { exact: true }).first()).toBeVisible();
  ...
  test('TC-07: Click Edit Personal Details', async ({ page }) => {
    ...
    // ASSERT (BLOCKING) Mobile Number field is now an editable input, Save button visible
    await expect(mobileInput(page)).toBeVisible();
    await expect(saveButton(page)).toBeVisible();
  });

  test('TC-08: Click inside the Mobile Number field', async ({ page }) => {
    test.setTimeout(180_000);
    await loginAsKwena(page);
    await openTargetApplication(page);
    await clickEditPersonalDetails(page);
    const input = mobileInput(page);
    await input.click();
    // ASSERT (BLOCKING) Mobile Number field is focused
    await expect(input).toBeFocused();
  });

  test('TC-09: Clear the Mobile Number field', async ({ page }) => {
    test.setTimeout(180_000);
    await loginAsKwena(page);
    await openTargetApplication(page);
    await clickEditPersonalDetails(page);
    ...
    await input.click();
    await input.fill('');
    // ASSERT (BLOCKING) Mobile Number field is empty
    await expect(input).toHaveValue('');
  });

  test('TC-10: Enter a different mobile number, "0821234567"', async ({ page }) => {
    test.setTimeout(180_000);
    await loginAsKwena(page);
    await openTargetApplication(page);
    await clickEditPersonalDetails(page);
    ...
    await input.click();
    await input.fill('');
    await input.fill(NEW_MOBILE);
    // ASSERT (BLOCKING) Mobile Number field contains the typed value
    await expect(input).toHaveValue(NEW_MOBILE);
  });
  ...
  test('TC-11: Click on Save', async ({ page }) => {
    ...
    // ASSERT (BLOCKING) after Save + reload, Mobile Number shows the updated value
    await page.reload({ waitUntil: 'domcontentloaded' });
    await page.waitForLoadState('networkidle').catch(() => {});
    await page.waitForTimeout(2000);
    await expect(page.getByText(NEW_MOBILE, { exact: true }).first()).toBeVisible({ timeout: 15_000 });

    // ASSERT (BLOCKING) the Applications index table's Cell Nr column also reflects the new mobile
