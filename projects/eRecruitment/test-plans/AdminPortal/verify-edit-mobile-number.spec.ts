// AUTO-RECORDED from test-plans/AdminPortal/verify-edit-mobile-number.md
// Source: Azure DevOps project pd-recruitment, test case #106532 "Verify
// Edit mobile number" (work item rev 2). The .md plan is canonical.
...
// ADO. TC-11 clicks Save for real, permanently changing the candidate's
// Mobile Number — confirmed with the requester before running. All
...
const NEW_MOBILE_NUMBER = '0837654321';
...
function mobileNumberInput(page: Page) {
  return page.locator('label', { hasText: 'Mobile Number' }).locator('xpath=../..').locator('input').first();
}
...
test.describe('ADMINPORTAL-106532 — Verify Edit mobile number', () => {
  ...
  test('TC-07: Click Edit Personal Details', async ({ page }) => {
    ...
    await expect(mobileNumberInput(page)).toBeVisible();
  ...
  test('TC-08: Click inside the Mobile Number field', async ({ page }) => {
    ...
    await openTargetApplication(page);
    await clickEditPersonalDetails(page);
    const input = mobileNumberInput(page);
    await input.click();
    // ASSERT (BLOCKING) Mobile Number field is focused and editable
  ...
  test('TC-10: Enter a different mobile number', async ({ page }) => {
    await loginAsKwena(page);
    await openTargetApplication(page);
    await clickEditPersonalDetails(page);
    const input = mobileNumberInput(page);
    await input.click();
    await input.fill('');
    await input.fill(NEW_MOBILE_NUMBER);
    // ASSERT (BLOCKING) Mobile Number field contains the typed value
    await expect(input).toHaveValue(NEW_MOBILE_NUMBER);
  });
  ...
  test('TC-11: Click on Save', async ({ page }) => {
    ...
    await page.waitForLoadState('networkidle').catch(() => {});
    await page.waitForTimeout(2000);
    await expect(page.getByText(NEW_MOBILE_NUMBER, { exact: false }).first()).toBeVisible({ timeout: 15_000 });

    // ASSERT (BLOCKING) the new Mobile Number also appears in the Applications index table (Cell Nr column)
    await openTargetJobPosting(page);
    await page.mouse.wheel(0, 1500);
