// AUTO-RECORDED from test-plans/AdminPortal/verify-edit-nature-of-disability-completeflow.md
// Source: Azure DevOps project pd-recruitment, test case #106268 "Verify
...
// Current Nature Of Disability value is "Test disability note" (set
// during the #106172 wizard).
//
...
// ADMINPORTAL-106240/106246/106247/106251/106255/106256/106257/106262).
...
//
// TC-10 clicks Save for real, changing Nature Of Disability from "Test
// disability note" to "Edit Disability" — confirmed with the requester
...
const CURRENT_DISABILITY_NOTE = 'Test disability note';
const NEW_DISABILITY_NOTE = 'Edit Disability';
...
function disabilityInput(page: Page) {
  return page.locator('label', { hasText: 'Nature Of Disability' }).first().locator('xpath=../..').locator('input, textarea').first();
}
...
test.describe('ADMINPORTAL-106268 — Verify Edit Nature of disability', () => {
  ...
  test('TC-06: Open the application created on Test Case 106172', async ({ page }) => {
    ...
    // ASSERT (BLOCKING) Personal Details panel visible with the current Nature Of Disability
    await expect(page.getByText('Personal Details', { exact: true }).first()).toBeVisible();
    await expect(page.getByText(CURRENT_DISABILITY_NOTE, { exact: false }).first()).toBeVisible();
  ...
  test('TC-07: Click Edit Personal Details', async ({ page }) => {
    ...
    // ASSERT (BLOCKING) Nature Of Disability field is now an editable input, Save button visible
    await expect(disabilityInput(page)).toBeVisible();
    await expect(saveButton(page)).toBeVisible();
  });

  test('TC-08: Clear the Nature Of Disability field', async ({ page }) => {
    test.setTimeout(180_000);
    await loginAsKwena(page);
    await openTargetApplication(page);
    await clickEditPersonalDetails(page);
    const input = disabilityInput(page);
    await input.click();
    await input.fill('');
    // ASSERT (BLOCKING) Nature Of Disability field is empty
    await expect(input).toHaveValue('');
  });

  test('TC-09: Enter "Edit Disability"', async ({ page }) => {
    test.setTimeout(180_000);
    await loginAsKwena(page);
    await openTargetApplication(page);
    await clickEditPersonalDetails(page);
    ...
    await input.click();
    await input.fill('');
    await input.fill(NEW_DISABILITY_NOTE);
    // ASSERT (BLOCKING) Nature Of Disability field contains the typed value
    await expect(input).toHaveValue(NEW_DISABILITY_NOTE);
  });
  ...
  test('TC-10: Click on Save', async ({ page }) => {
    ...
    // ASSERT (BLOCKING) after Save + reload, Nature Of Disability shows the updated value
    await page.reload({ waitUntil: 'domcontentloaded' });
    await page.waitForLoadState('networkidle').catch(() => {});
    await page.waitForTimeout(2000);
    await expect(page.getByText(NEW_DISABILITY_NOTE, { exact: true }).first()).toBeVisible({ timeout: 15_000 });
  });
});
