// AUTO-RECORDED from test-plans/AdminPortal/verify-edit-province-completeflow.md
// Source: Azure DevOps project pd-recruitment, test case #106269 "Verify
// Edit Province" (work item rev 3). The .md plan is canonical. AI-repair
...
// Current Province value is "Gauteng" (set during the #106172 wizard).
// ADO step 10's wording erroneously refers to "nature of disability text
// area" but the field and test title make the intent unambiguous
// (Province) — same class of copy/paste error seen elsewhere in this
// batch (e.g. ADMINPORTAL-106246).
//
...
// ADMINPORTAL-106240/106246/106247/106251/106255/106256/106257/106262/106268).
// This spec waits 15s after clicking Save and retries once if still in
// edit mode. See test-reports/bugs/2026-08-05-edit-personal-details-save-silently-fails.md.
//
// TC-10 clicks Save for real, changing Province from "Gauteng" to "Edited
// Province" — confirmed with the requester before running. All earlier
// TCs never click Save.
...
const CURRENT_PROVINCE = 'Gauteng';
const NEW_PROVINCE = 'Edited Province';
...
function provinceInput(page: Page) {
  return page.locator('label', { hasText: 'Province' }).first().locator('xpath=../..').locator('input').first();
}
...
test.describe('ADMINPORTAL-106269 — Verify Edit Province', () => {
  ...
  test('TC-06: Open the application created on Test Case 106172', async ({ page }) => {
    ...
    // ASSERT (BLOCKING) Personal Details panel visible with the current Province
    await expect(page.getByText('Personal Details', { exact: true }).first()).toBeVisible();
    await expect(page.getByText(CURRENT_PROVINCE, { exact: true }).first()).toBeVisible();
  ...
  test('TC-07: Click Edit Personal Details', async ({ page }) => {
    ...
    // ASSERT (BLOCKING) Province field is now an editable input, Save button visible
    await expect(provinceInput(page)).toBeVisible();
    await expect(saveButton(page)).toBeVisible();
  });

  test('TC-08: Clear the Province field', async ({ page }) => {
    test.setTimeout(180_000);
    await loginAsKwena(page);
    await openTargetApplication(page);
    await clickEditPersonalDetails(page);
    const input = provinceInput(page);
    await input.click();
    await input.fill('');
    // ASSERT (BLOCKING) Province field is empty
    await expect(input).toHaveValue('');
  });

  test('TC-09: Enter "Edited Province"', async ({ page }) => {
    test.setTimeout(180_000);
    await loginAsKwena(page);
    await openTargetApplication(page);
    await clickEditPersonalDetails(page);
    ...
    await input.click();
    await input.fill('');
    await input.fill(NEW_PROVINCE);
    // ASSERT (BLOCKING) Province field contains the typed value
    await expect(input).toHaveValue(NEW_PROVINCE);
  });
  ...
  test('TC-10: Click on Save', async ({ page }) => {
    ...
    // ASSERT (BLOCKING) after Save + reload, Province shows the updated value
    await page.reload({ waitUntil: 'domcontentloaded' });
    await page.waitForLoadState('networkidle').catch(() => {});
    await page.waitForTimeout(2000);
    await expect(page.getByText(NEW_PROVINCE, { exact: true }).first()).toBeVisible({ timeout: 15_000 });

    // ASSERT (BLOCKING) the new Province also appears in the Applications index table
