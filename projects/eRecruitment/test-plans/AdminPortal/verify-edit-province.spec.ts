// AUTO-RECORDED from test-plans/AdminPortal/verify-edit-province.md
// Source: Azure DevOps project pd-recruitment, test case #106537 "Verify
// Edit Province" (work item rev 2). The .md plan is canonical. AI-repair
...
// ADO. ADO step 10 has a copy/paste leftover from #106536 ("...natire of
// disability text area") — read as "Province". TC-10 clicks Save for
// real, permanently changing the candidate's Province (currently blank)
...
function provinceInput(page: Page) {
  return page.locator('label', { hasText: 'Province' }).locator('xpath=../..').locator('input, textarea').first();
}
...
test.describe('ADMINPORTAL-106537 — Verify Edit Province', () => {
  ...
  test('TC-10: Click on Save', async ({ page }) => {
    ...
    await page.waitForLoadState('networkidle').catch(() => {});
    await page.waitForTimeout(2000);
    await expect(page.getByText(NEW_PROVINCE, { exact: false }).first()).toBeVisible({ timeout: 15_000 });
