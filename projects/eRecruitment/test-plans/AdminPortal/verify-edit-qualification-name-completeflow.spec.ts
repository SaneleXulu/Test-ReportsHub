// AUTO-RECORDED from test-plans/AdminPortal/verify-edit-qualification-name-completeflow.md
// Source: Azure DevOps project pd-recruitment, test case #106278 "Verify
// Edit qualification name" (work item rev 2). The .md plan is canonical.
...
// Qualification Name "BSc Automation". ADO steps 10-11 have a
// copy/paste leftover ("...Institution text area") — read as
// "Qualification Name" per step 9 and the test title (same class of
// error as ADMINPORTAL-106541).
//
// Unlike Institution (a required field that rejects a blank save, see
// ADMINPORTAL-106276/106540), Qualification Name is populated with a real
// replacement value here, so the Save is expected to succeed and persist
// — confirmed with the requester before running.
//
// scrollIntoViewIfNeeded() is unstable here (content above still
// loading) — use a plain wheel scroll instead (see ADMINPORTAL-106276).
//
...
// test (same fix as ADMINPORTAL-106540/106541/106542/106547/106276).
...
// Row anchor: Institution, stable throughout this edit (unlike the field
// under test, Qualification Name, which changes during the test).
...
async function findTargetRowIndex(page: Page): Promise<number> {
  ...
  // to the DOM" as the page re-renders mid-scroll (see ADMINPORTAL-106276).
...
function qualificationNameInputAt(page: Page, index: number) {
  // Column order in the Secondary/Tertiary Qualifications table is
  // Institution, Qualification Name, Qualification Type, Certificate —
  // Qualification Name is the second plain <input>.
...
test.describe('ADMINPORTAL-106278 — Verify Edit qualification name', () => {
  ...
  test('TC-10: Populate the field with "Edited Qualification Name"', async ({ page }) => {
    test.setTimeout(180_000);
    await loginAsKwena(page);
    await openTargetApplication(page);
    const index = await enterEducationEditMode(page);
