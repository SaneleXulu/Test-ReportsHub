// AUTO-RECORDED from test-plans/AdminPortal/verify-edit-qualification-status-completeflow.md
// Source: Azure DevOps project pd-recruitment, test case #106280 "Verify
// edit qualification status (In Progress)" (work item rev 3). The .md
// plan is canonical. AI-repair will patch failing lines in this file.
...
// wizard — genuinely HAS a Qualification Status dropdown in edit mode
// (confirmed live 2026-08-06: 2 .ant-select elements in the row —
// Qualification Type first, Qualification Status second, currently
// "Complete"). This is a real, executable test case for this
// application specifically.
//
// scrollIntoViewIfNeeded() is unstable here (content above still
// loading) — use a plain wheel scroll instead (see ADMINPORTAL-106276/106279).
//
...
// test (same fix as ADMINPORTAL-106540/106541/106542/106547/106276/106278/106279).
//
// TC-10 clicks the row's inline Save icon for real, changing
// Qualification Status from "Complete" to "In Progress" — confirmed with
// the requester before running.
...
const NEW_QUALIFICATION_STATUS = 'In Progress';
...
async function selectInProgress(page: Page, index: number) {
...
test.describe('ADMINPORTAL-106280 — Verify edit qualification status (In Progress)', () => {
  ...
  test('TC-09: Select "In Progress"', async ({ page }) => {
    test.setTimeout(180_000);
    await loginAsKwena(page);
    await openTargetApplication(page);
    const index = await enterEducationEditMode(page);
    await selectInProgress(page, index);
    // ASSERT (BLOCKING) Qualification Status dropdown now shows "In Progress"
