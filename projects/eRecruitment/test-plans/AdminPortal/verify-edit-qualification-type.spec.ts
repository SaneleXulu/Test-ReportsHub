// AUTO-RECORDED from test-plans/AdminPortal/verify-edit-qualification-type.md
// Source: Azure DevOps project pd-recruitment, test case #106542 "Verify
// Edit qualification type" (work item rev 3). The .md plan is
...
// set by ADMINPORTAL-106541). Confirmed live 2026-08-05: unlike the
// Gender/Race/Internal-External dropdowns elsewhere in this project, the
// Qualification Type dropdown lists ALL 5 options including the
...
// within the same test (same fix as ADMINPORTAL-106540/106541).
//
...
// and persist — confirmed with the requester before running. All earlier
...
const NEW_QUALIFICATION_TYPE = 'Grade 12 (National Senior Certificate) and National (vocational) Cert. level 4';
...
async function findTargetRowIndex(page: Page): Promise<number> {
  // Avoid scrollIntoViewIfNeeded() here — the page above the Education
  // panel (Personal Details selects, etc.) keeps shifting height while it
  // loads, which repeatedly fails scrollIntoViewIfNeeded's "element is
  // stable" check (see this test case's first two attempts). waitFor
  // 'visible' doesn't require the element to be scrolled into the
  // viewport, and the row's own .click() calls later auto-scroll via a
  // more robust mechanism.
  await page.getByText(ROW_ANCHOR_TEXT, { exact: false }).first().waitFor({ state: 'visible', timeout: 30_000 });
...
function qualificationTypeSelectAt(page: Page, index: number) {
  // Qualification Type is the only ant-select combobox in this row.
...
test.describe('ADMINPORTAL-106542 — Verify Edit qualification type', () => {
  ...
  test('TC-08: Click on the Qualification Type dropdown', async ({ page }) => {
    ...
    await qualificationTypeSelectAt(page, index).click();
    await page.waitForTimeout(500);
    // ASSERT (BLOCKING) all 5 Qualification Type options are displayed
    await expect(page.getByText('Grade 9', { exact: true }).first()).toBeVisible();
    await expect(page.getByText('Grade 10 and National (vocational) Certificates level 2', { exact: true }).first()).toBeVisible();
    await expect(page.getByText('Grade 11 and National (vocational) Certificates level 3', { exact: true }).first()).toBeVisible();
    await expect(page.getByText(NEW_QUALIFICATION_TYPE, { exact: true }).first()).toBeVisible();
    await expect(page.getByText('Higher Certificates and Advanced National (vocational) Cert.', { exact: true }).first()).toBeVisible();
  });

  test('TC-09: Select "Grade 12 (National Senior Certificate) and National (vocational) Cert. level 4"', async ({ page }) => {
    await loginAsKwena(page);
    await openTargetApplication(page);
    const index = await enterEducationEditMode(page);
