// AUTO-RECORDED from test-plans/AdminPortal/verify-edit-race.md
// Source: Azure DevOps project pd-recruitment, test case #106535 "Verify
// Edit Race" (work item rev 2). The .md plan is canonical. AI-repair
...
// ADO; the Race dropdown's option list (with "African" selected) shows
// "Not Stated", "White", "Coloured", "Indian", "Other". TC-10 clicks Save
// for real, permanently changing the candidate's Race — confirmed with
// the requester before running. All earlier TCs never click Save.
...
function raceSelect(page: Page) {
  return page.locator('label', { hasText: 'Race' }).locator('xpath=../..').locator('.ant-select').first();
}
...
test.describe('ADMINPORTAL-106535 — Verify Edit Race', () => {
  ...
  test('TC-08: Click on the Race dropdown', async ({ page }) => {
    ...
    await expect(page.getByText('Not Stated', { exact: true }).first()).toBeVisible();
    await expect(page.getByText('White', { exact: true }).first()).toBeVisible();
    await expect(page.getByText('Coloured', { exact: true }).first()).toBeVisible();
  ...
  test('TC-10: Click on Save', async ({ page }) => {
    ...
    // ASSERT (BLOCKING) the new Race also appears in the Applications index table (Race column)
    await openTargetJobPosting(page);
    await page.mouse.wheel(0, 1500);
