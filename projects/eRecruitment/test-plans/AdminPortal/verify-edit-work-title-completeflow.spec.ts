// AUTO-RECORDED from test-plans/AdminPortal/verify-edit-work-title-completeflow.md
// Source: Azure DevOps project pd-recruitment, test case #106285 "Verify
// Edit Work Title" (work item rev 4). The .md plan is canonical. AI-repair
...
// Applications table's Surname/Initials column, Work Experience row
// "QA Automation Engineer" / "Test Employer Pty Ltd".
//
// Discrepancy vs. ADMINPORTAL-106547 (same test case type, different
// candidate): that spec only cleared Job Title to empty, which required-
// field validation rejects (no persistent change). This test case's ADO
// steps explicitly populate a replacement value ("Edited Job title")
// before saving, so this IS a real, valid, persistent edit — confirmed
// with the requester before running TC-11's real Save click.
//
...
// (same fix as ADMINPORTAL-106540/106541/106542/106547).
...
const NEW_JOB_TITLE = 'Edited Job title';
...
const ROW_ANCHOR_TEXT = 'QA Automation Engineer';
...
test.describe('ADMINPORTAL-106285 — Verify Edit Work Title', () => {
  ...
  test('TC-10: Populate the field with "Edited Job title"', async ({ page }) => {
    test.setTimeout(180_000);
    await loginAsKwena(page);
    await openTargetApplication(page);
    const index = await enterWorkExperienceEditMode(page);
    ...
    await input.click();
    await input.fill('');
    await input.fill(NEW_JOB_TITLE);
    // ASSERT (BLOCKING) Job Title field contains the typed value
    await expect(input).toHaveValue(NEW_JOB_TITLE);
  });
  ...
  test('TC-11: Click on Save', async ({ page }) => {
    ...
    await page.waitForTimeout(2000);
    // ASSERT (BLOCKING) after Save + reload, the row shows the updated Job Title
    await page.reload({ waitUntil: 'domcontentloaded' });
    await page.waitForLoadState('networkidle').catch(() => {});
    await page.waitForTimeout(2000);
    await page.mouse.wheel(0, 2200);
    await page.getByText(NEW_JOB_TITLE, { exact: false }).first().waitFor({ state: 'visible', timeout: 60_000 });
    const rows = page.locator('div[role="row"]');
    const count = await rows.count();
    let found = false;
    for (let i = 0; i < count; i++) {
      const text = await rows.nth(i).innerText().catch(() => '');
      if (text.includes(NEW_JOB_TITLE)) { found = true; break; }
    }
