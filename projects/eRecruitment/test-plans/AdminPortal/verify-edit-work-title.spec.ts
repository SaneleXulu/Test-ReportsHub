// AUTO-RECORDED from test-plans/AdminPortal/verify-edit-work-title.md
// Source: Azure DevOps project pd-recruitment, test case #106547 "Verify
// Edit Work Title" (work item rev 4). The .md plan is canonical.
...
// for "Senior Test Engineer" / ABSA. Confirmed live 2026-08-05: Job Title
// is a required field, same as Institution in ADMINPORTAL-106540 —
// clicking Save with it blank is REJECTED ("Update failed" tooltip, no
// API update call fires), so this is not actually a persistent edit
// despite clicking the real Save control. No confirmation was needed to
// run this test end-to-end as a result.
//
...
// within the same test (same fix as ADMINPORTAL-106540/106541/106542).
...
async function findTargetRowIndex(page: Page): Promise<number> {
  ...
  // ADMINPORTAL-106544/106545's attempts), this is a real slowness
  // workaround, not a selector fix.
...
function jobTitleInputAt(page: Page, index: number) {
  // Column order is Job Title, Employer, Employment Start Date,
  // Employment End Date, Reason For Leaving — Job Title is the first
  // plain <input>.
...
function saveIconAt(page: Page, index: number) {
  ...
  // unambiguous class for the row's inline save action (as opposed to
  // date-picker/select suffix icons also present in the row).
  return page.locator('div[role="row"]').nth(index).locator('.anticon-save').first();
}

test.describe('ADMINPORTAL-106547 — Verify Edit Work Title', () => {
  ...
  test('TC-07: Navigate to Work Experience panel and click the Edit icon', async ({ page }) => {
    ...
    // ASSERT (BLOCKING) the row's Job Title field is now an editable input
    await expect(jobTitleInputAt(page, index)).toBeVisible();
  });

  test('TC-08: Click on the Job Title text area', async ({ page }) => {
    test.setTimeout(180_000);
    await loginAsKwena(page);
    await openTargetApplication(page);
    const index = await enterWorkExperienceEditMode(page);
    const input = jobTitleInputAt(page, index);
    await input.click();
    // ASSERT (BLOCKING) Job Title field is focused and editable
    await expect(input).toBeFocused();
  });

  test('TC-09: Clear the Job Title text area', async ({ page }) => {
    test.setTimeout(180_000);
    await loginAsKwena(page);
    await openTargetApplication(page);
    const index = await enterWorkExperienceEditMode(page);
    ...
    await input.click();
    await input.fill('');
    // ASSERT (BLOCKING) Job Title field is empty
  ...
  test('TC-10: Click on Save', async ({ page }) => {
    ...
    await input.click();
    await input.fill('');
    // STEP: CLICK the row's inline Save icon (ADO calls it "OK", but this
    // is the same Save control used elsewhere). Job Title is required —
    // confirmed live this is REJECTED by validation, so no persistent
    // change actually occurs.
    await saveIconAt(page, index).click();
    await page.waitForTimeout(1500);
    // ASSERT (BLOCKING) an "Update failed" indicator appears
    await expect(page.getByText('Update failed', { exact: false }).first()).toBeVisible({ timeout: 10_000 });
    // ASSERT (BLOCKING) after reload, the original Job Title is retained (save was rejected)
