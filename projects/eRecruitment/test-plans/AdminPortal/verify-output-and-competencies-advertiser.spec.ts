// AUTO-RECORDED from test-plans/AdminPortal/verify-output-and-competencies-advertiser.md
// Source: Azure DevOps project pd-recruitment, test case #108069 "Verify
// Output and Competencies" (steps defined inline, NOT via the shared-step
// block used by #102822 and its siblings — work item rev 1).
// The .md plan is canonical. AI-repair will patch failing lines in this file.
// Do not hand-edit unless you are also updating the .md plan.
//
// READ-ONLY: same Advertiser flow as
// AdminPortal/verify-job-information-summary-details-advertiser.spec.ts
// (login as Moshadih, target a specific job with confirmed complete data
// rather than blindly "the first" Advertise Job Posting row — see that
// spec's comment for why). Confirmed live 2026-08-05: ADO step 5's expected
// buttons are copy-pasted from the Authoriser flow and don't match this
// page (Close/View in PDF only). Requirements / Required Skills and
// Competencies / Duties are plain read-only
// `<span class="read-only-display-form-item">` fields (not nested in a
// nested subform like Salary Range on the Job Information Summary tab), so
// the standard label -> sibling `.ant-form-item-control` lookup works
// correctly here.
...
test.describe('ADMINPORTAL-108069 — Verify Output and Competencies', () => {
  ...
  test('TC-05: Click on Output and Competency Profiles tab', async ({ page }) => {
    await loginAsMoshadi(page);
    await goToInbox(page);
    await openTargetAdvertiseJobItem(page);
    ...
    await expect(fieldControlText(page, 'Requirements')).toBeVisible();
  });
  ...
  test('TC-06: Check if the requirements field is populated', async ({ page }) => {
    ...
    await expect(fieldControlText(page, 'Requirements')).not.toHaveText('');
  });
  ...
  test('TC-07: Check that required skills and competencies is populated', async ({ page }) => {
    ...
    await expect(fieldControlText(page, 'Required Skills and Competencies')).not.toHaveText('');
  });

  test('TC-08: Check that Duties field is populated', async ({ page }) => {
    await advanceToOutputAndCompetencyTab(page);
    // ASSERT (BLOCKING) Duties field is non-empty
    await expect(fieldControlText(page, 'Duties')).not.toHaveText('');
  });
});
