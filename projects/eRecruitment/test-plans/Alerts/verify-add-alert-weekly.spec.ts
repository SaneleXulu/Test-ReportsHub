// AUTO-RECORDED from test-plans/Alerts/verify-add-alert-weekly.md
// Source: Azure DevOps test plan #99437, suite #104540, test case #106343
// The .md plan is canonical. AI-repair will patch failing lines in this file.
// Do not hand-edit unless you are also updating the .md plan.
//
// Add Alert dialog DOM confirmed live 2026-07-30:
...
// - Day Of The Week: <label for="dayOfWeek"> + Ant Select combobox, wrapped
//   in ant-form-item-hidden until Weekly is selected.
...
function weeklyRadio(page: Page): Locator {
  return addAlertDialog(page).locator('.ant-radio-wrapper').filter({ hasText: 'Weekly' }).locator('input[type="radio"]');
}

function dayOfWeekFormItem(page: Page): Locator {
  return formItemByLabel(page, 'dayOfWeek');
}

function dayOfWeekSelect(page: Page): Locator {
  return dayOfWeekFormItem(page).locator('.ant-select').first();
}
...
test.describe('ALERTS-106343 — Verify Add an Alert (Weekly)', () => {
  ...
  test('TC-09: Click Weekly radio button', async ({ page }) => {
    await openAddAlertDialog(page);
    // STEP: CLICK the Weekly radio button
    await weeklyRadio(page).check();
    // ASSERT (BLOCKING) Weekly radio selected; Day Of The Week dropdown enabled
    await expect(weeklyRadio(page)).toBeChecked();
    await expect(dayOfWeekFormItem(page)).not.toHaveClass(/ant-form-item-hidden/, { timeout: 10000 });
  });

  test('TC-10: Click Day Of The Week dropdown', async ({ page }) => {
    await openAddAlertDialog(page);
    await weeklyRadio(page).check();
    await expect(dayOfWeekFormItem(page)).not.toHaveClass(/ant-form-item-hidden/, { timeout: 10000 });
    // STEP: CLICK the Day Of The Week dropdown
    await dayOfWeekSelect(page).click();
    // ASSERT (BLOCKING) list of day options is displayed
    await expect(dropdownList(page)).toBeVisible({ timeout: 10000 });
  });

  test('TC-11: Select a day, e.g. Friday', async ({ page }) => {
    await openAddAlertDialog(page);
    await weeklyRadio(page).check();
    await expect(dayOfWeekFormItem(page)).not.toHaveClass(/ant-form-item-hidden/, { timeout: 10000 });
    // STEP: SELECT "Friday"
    await selectAntOption(page, dayOfWeekSelect(page), DAY_OF_WEEK);
    // ASSERT (BLOCKING) Day Of The Week field displays "Friday"
    await expect(dayOfWeekSelect(page).locator('.ant-select-selection-item')).toHaveText(DAY_OF_WEEK, { timeout: 10000 });
  });

  test('TC-12: Click OK button', async ({ page }) => {
    await openAddAlertDialog(page);
    await keywordsField(page).fill(JOB_TITLE);
    await selectAntOption(page, locationSelect(page), LOCATION);
    await minSalaryField(page).fill(MIN_SALARY);
