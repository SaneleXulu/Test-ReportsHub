// AUTO-RECORDED from test-plans/Home/search-with-job-title-only.md
// Source: Azure DevOps test plan #99437, suite #106554, test case #106558
...
test.describe('HOME-106558 — Search with Job Title Only', () => {
  ...
  test('TC-03: Enter a job title, e.g. Software Engineer', async ({ page }) => {
    await loginAsFred(page);
    await goToHome(page);
    // STEP: TYPE "Software Engineer" into the Job Title field
  ...
  test('TC-05: Click on Search button', async ({ page }) => {
    ...
    await page.waitForLoadState('networkidle');
    // ASSERT (BLOCKING) search results are displayed matching the job title
