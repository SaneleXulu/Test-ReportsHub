// AUTO-RECORDED from test-plans/Profile/verify-complete-secondary-qualifications.md
// Source: Azure DevOps test plan #99437, suite #104586, test case #104623
// The .md plan is canonical. AI-repair will patch failing lines in this file.
// Do not hand-edit unless you are also updating the .md plan.
//
// Selectors were captured live against the QA environment on 2026-07-06.
// Unlike Languages, Secondary Qualifications is a single-record form (no
// add-to-table "+" button), so values are simply overwritten each run.
...
test.describe('PROFILE-104623 — Verify Complete Secondary Qualifications', () => {
  ...
  test('TC-05: Qualification Status Complete reveals Date Obtained', async ({ page }) => {
    await loginAsFred(page);
    await goToSecondaryQualifications(page);
    ...
    // "Complete" is the 2nd of 2 options; its DOM node reports as hidden
    // (same rc-virtual-list quirk as Qualification Type), so a click times
    // out. Keyboard nav is reliable: ArrowDown highlights In Progress, a
    // second ArrowDown highlights Complete, Enter confirms.
    ...
    const dateField = fieldInput(page, 'Date Obtained');
    await expect(dateField).toBeVisible();
    await expect(dateField).toBeEnabled();
  });

  test('TC-06: Date Obtained picker and Next navigation', async ({ page }) => {
    await loginAsFred(page);
    await goToSecondaryQualifications(page);
    const qualStatusCombo = antCombo(page, 'Qualification Status');
    await qualStatusCombo.click();
