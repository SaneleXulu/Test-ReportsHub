// AUTO-RECORDED from test-plans/Profile/verify-delete-work-experience.md
// Source: Azure DevOps test plan #99437, suite #104586, test case #104642
...
// with Cancel/OK buttons, same as verify-delete-tertiary-qualification.spec.ts.
...
test.describe('PROFILE-104642 — Delete Work Experience', () => {
  ...
  test('TC-08: Click Delete icon on the added Work Experience', async ({ page }) => {
    await loginAsFred(page);
    await goToWorkExperience(page);
    await addWorkExperience(page);
  ...
  test('TC-09: Click OK button', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'OK' }).click();
    await page.waitForTimeout(1000);
    // ASSERT (BLOCKING) the deleted row's company name is no longer visible
    await expect(page.getByText(COMPANY_NAME, { exact: false })).toHaveCount(0);
  });
});
